...
def test_walk_follows_edges():
    ...
    # first 无任何邻居 → 只能重新抽签，second 任意
    had_no_neighbors = not g.outgoing.get(first) and not g.incoming.get(first)
    assert adjacent or had_no_neighbors
